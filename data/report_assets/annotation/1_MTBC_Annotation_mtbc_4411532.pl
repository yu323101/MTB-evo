...
my $tuberculist = File::Spec->catfile($Bin, "2_Tuberculist_new_20150307");
open F1, "<$tuberculist" or die $!; #open Tuberculist_new_20150307
while(<F1>){
	chomp;
	@a=split "\t",$_;
	$start{$a[0]}=$a[3];
...
close F1;
$last_igr="Rv3924c-Rv0001";
$igrha{$last_igr}=1;
push(@igr,$last_igr);
$start{$last_igr}=4410930;
$end{$last_igr}=4411532;

@all=(@gene,@igr);

my $genetic_codes = File::Spec->catfile($Bin, "3_genetic_codes");
open F2, "<$genetic_codes" or die $!; #open genetic_code
while(<F2>){
	chomp;
	@a=split "\t",$_;
	$code{$a[0]}=$a[1];
...
my $genome_fasta = File::Spec->catfile($Bin, "4_mtbc_sequence.fasta");
open F3, "<$genome_fasta" or die $!; #open genome.fasta
while(<F3>){
	chomp;
	if($_=~m/>/){
		}
